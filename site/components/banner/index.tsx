...
const Banner: React.FC<SlideShowProps> = ({ images }) => {
  ...
  return (
    <section className="w-[100%] h-[600px]  bg-black relative">
      <div className=" absolute w-full h-full">
        {images.map((img, index) => (
          <img 
            ...
            src={img} 
            alt={`Slide ${index + 1}`}
            className={`absolute w-[100%] h-[600px] object-cover transition-opacity duration-1000 ${curSlide !== index && 'opacity-0'}`}
          />
        ))}
      </div>
